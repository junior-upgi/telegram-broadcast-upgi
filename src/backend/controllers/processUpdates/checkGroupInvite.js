module.exports = (args) => {
    if (
        ...
        (args.update.message.new_chat_member !== undefined) &&
        // invitation is to the designated bot
        (
            (args.update.message.new_chat_member.id === args.targetBot.id) ||
            (args.update.message.new_chat_member.username === args.targetBot.username)
        )
    )
        return true;
    else
